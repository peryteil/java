...
public class FruitList {
    public static void main(String[] args) {
        ...
        // 사과 맛을 "사과본연의 맛"으로 수정하기
        // name = 사고인 것을 찾아서 taste 를 수정한다.
        for (Fruit x : fruits) {
        ...
        System.out.println(fruits);
        // 망고를 사과로 바꾸기(망고위치 찾고 사과로 바꾸기)
        for (Fruit x : fruits) {
            if (x.getName().equals("망고")) {
                int upodate = fruits.indexOf(x);
                // 수정하기
                fruits.set(upodate, new Apple());
            }
            break;
        }
        System.out.println(fruits);
